...
class ClassAssignment:
    ...
    def set_class_size_constraints(self):
        for c in self.classes:
            self.problem += pulp.lpSum([self.assignment_vars[s, c] for s in self.students]) == self.students_per_class
    ...
    def set_gender_balance_constraints(self):
        ...
        for c in self.classes:
            self.problem += pulp.lpSum(self.assignment_vars[s, c] for s in self.students if self.student_df.loc[s-1, 'gender'] == 1) == males_per_class
            self.problem += pulp.lpSum(self.assignment_vars[s, c] for s in self.students if self.student_df.loc[s-1, 'gender'] == 0) == females_per_class
    ...
    def set_leader_constraints(self):
        ...
        leaders_per_class = len(leaders) // len(self.classes)
        for c in self.classes:
    ...
    def set_support_constraints(self):
        ...
        supports_per_class = len(supports) // len(self.classes)
        for c in self.classes:
